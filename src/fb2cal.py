...
class Birthday:
    ...
    def __str__(self):
        return "{0.name} ({0.day}/{0.month})".format(self)
...
def main():
    ...
    try:
        ...
        if not dataset:
            logger.error(
                f"{CONFIG_FILE_PATH} does not exist. Please rename {CONFIG_FILE_TEMPLATE_NAME} if you have not done so already."
            )
            raise SystemExit
    ...
    try:
        ...
        logger.error(
            f'Invalid logging level specified. Level: {config["LOGGING"]["level"]}'
        )
        raise SystemError
    ...
    facebook_authenticate(
        browser, config["AUTH"]["FB_EMAIL"], config["AUTH"]["FB_PASS"]
    )
...
def facebook_authenticate(browser, email, password):
    ...
    if login_page.status_code != 200:
        ...
        logger.error(
            f"Failed to authenticate with Facebook with email {email}. Stage: Initial Request for datr Token, Status code: {login_page.status_code}."
        )
        raise SystemError
    ...
    if not matches or len(matches.groups()) != 1:
        logger.debug(login_page.text)
        logger.error(
            f"Match failed or unexpected number of regexp matches when trying to get datr token."
        )
        raise SystemError
    ...
    if login_page.status_code != 200:
        logger.debug(login_page.text)
        logger.error(
            f"Failed to authenticate with Facebook with email {email}. Stage: Main Login Attempt, Status code: {login_page.status_code}."
        )
        raise SystemError
    ...
    if login_response.status_code != 200:
        ...
        logger.error(
            f"Failed to authenticate with Facebook with email {email}. Stage: Main Login Reponse, Status code: {login_response.status_code}."
        )
        raise SystemError

    # Check to see if login failed
    if login_response.soup.find(
        "link", {"rel": "canonical", "href": "https://www.facebook.com/login/"}
    ):
        logger.debug(login_response.text)
        logger.error(
            f"Failed to authenticate with Facebook with email {email}. Please check provided email/password."
        )
        raise SystemError
...
def get_async_token(browser):
    ...
    FACEBOOK_ASYNC_TOKEN_REGEXP_STRING = (
    ...
    if birthday_event_page.status_code != 200:
        ...
        logger.error(
            f"Failed to retreive birthday event page. Status code: {birthday_event_page.status_code}."
        )
        raise SystemError
    ...
    if not matches or len(matches.groups()) != 1:
        logger.debug(birthday_event_page.text)
        logger.error(
            f"Match failed or unexpected number of regexp matches when trying to get async token."
        )
        raise SystemError
...
def get_facebook_locale(browser):
    ...
    query_params = {"fb_dtsg_ag": get_async_token(browser), "__a": "1"}

    response = browser.get(
        FACEBOOK_LOCALE_ENDPOINT + urllib.parse.urlencode(query_params)
    )

    if response.status_code != 200:
        logger.debug(response.text)
        logger.error(
            f"Failed to get Facebook locale. Params: {query_params}. Status code: {response.status_code}."
        )
        raise SystemError
...
def get_async_birthdays(browser):
    ...
    for epoch_timestamp in next_12_months_epoch_timestamps:
        logger.info(
            f'Processing birthdays for month {datetime.fromtimestamp(epoch_timestamp).strftime("%B")}.'
        )
        ...
        if response.status_code != 200:
            logger.debug(response.text)
            logger.error(
                f"Failed to get async birthday response. Params: {query_params}. Status code: {response.status_code}."
            )
            raise SystemError
        ...
        logger.info(
            f'Found {len(birthdays_for_month)} birthdays for month {datetime.fromtimestamp(epoch_timestamp).strftime("%B")}.'
        )
